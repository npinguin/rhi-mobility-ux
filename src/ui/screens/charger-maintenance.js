    ...
    this.shadowRoot.querySelectorAll("button[data-charger-picker-save]").forEach((btn) => {
      btn.addEventListener("click", async (ev) => {
        ...
        try {
          ...
          if (!await rt.writePublishedPropertyAsync(assetId, "charger.image_key", key)) throw new Error("appearance write unavailable");
          btn.classList.remove("pending"); btn.classList.add("sent");
          this._chargerPickerDraft.delete(assetId);
          this._chargerPickerAsset=""; this._forceRender=true; this._lastSignature="";
          if(this._hass)this.hass=this._hass;
        } catch (error) {
          btn.disabled=false; btn.classList.remove("pending");
          console.error("RHI Mobility charger picker save failed",error);
        }
      });
    });
...
@media(max-width:900px){.page{padding:14px}.hero{grid-template-columns:1fr;padding:22px}.hero-side{grid-template-columns:1fr}.hero .charger-hero-visual{display:none}.hero-metrics{grid-template-columns:repeat(3,1fr)}h1{font-size:36px}.grid{grid-template-columns:1fr}.charger-hero-card{grid-template-columns:1fr}.charger-kpis{grid-template-columns:1fr}.charger-head{grid-template-columns:44px minmax(0,1fr);}.status{grid-column:1/-1;justify-self:start}.command-row{grid-template-columns:1fr 1fr}}
      ...
      .charger-picker-panel{margin:0 12px 10px;border:1px solid #cfe0f6;border-radius:14px;background:linear-gradient(135deg,#fbfdff,#f3f8ff);padding:12px 14px;box-shadow:inset 0 1px 0 rgba(255,255,255,.8)}
      ...
      .charger-picker-panel .vehicle-picker-close{width:30px;height:30px;border:1px solid #dbe5f0;border-radius:8px;background:#fff;color:#64748b;cursor:pointer}
      ...
      .charger-picker-panel .vehicle-picker-grid label,.charger-picker-panel .vehicle-picker-key{display:flex;flex-direction:column;gap:4px}
      .charger-picker-panel .vehicle-picker-grid label>span,.charger-picker-panel .vehicle-picker-key>span{font-size:8.5px;font-weight:700;color:#64748b;text-transform:uppercase;letter-spacing:.05em}
      ...
      .charger-picker-panel .vehicle-picker-key code{height:34px;display:flex;align-items:center;border:1px solid #d7e2ef;border-radius:8px;background:#fff;padding:0 8px;font-size:8.5px;color:#475569;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
      .charger-picker-panel .vehicle-picker-save{height:34px;border:1px solid #0b65ea;border-radius:8px;background:#0b65ea;color:#fff;padding:0 11px;display:flex;align-items:center;justify-content:center;gap:6px;font-size:10px;font-weight:700;cursor:pointer}
      .charger-picker-panel .vehicle-picker-save:disabled{background:#e2e8f0;border-color:#d5deea;color:#94a3b8;cursor:not-allowed}
      .charger-picker-panel .vehicle-picker-gap{margin-top:8px;display:flex;align-items:center;gap:6px;color:#9a5a16;font-size:9.5px}
      @media(max-width:900px){.charger-picker-panel .vehicle-picker-grid{grid-template-columns:1fr 1fr}.charger-picker-panel .vehicle-picker-key{grid-column:1/-1}.charger-picker-panel .vehicle-picker-save{justify-content:center}}
