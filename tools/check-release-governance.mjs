...
const sharedFooter=read('documentation/UX_FOOTER_STANDARD.md');
...
const requiredFiles=[
  ...
  'documentation/BRANDING.md','documentation/UX_RELEASE_STANDARD.md','documentation/UX_FOOTER_STANDARD.md','documentation/ENGINEER_HANDOVER.md','documentation/HACS_INSTALLATION.md',
...
if(status.hacs_visibility!=='normal_release_visible_without_beta_toggle') throw new Error('TEST CANDIDATE must be visible in normal HACS version selection');
...
if(!governance.includes('normal GitHub Releases, not GitHub prereleases')) throw new Error('HACS-visible candidate governance missing');
if(!sharedRelease.includes('TEST CANDIDATE is published as a normal GitHub Release')) throw new Error('shared UX release standard missing normal-release candidate rule');
if(!sharedFooter.includes('RHI <Module> UX <version> · Backend <version>')) throw new Error('shared UX footer standard missing');
...
if(publishWorkflow.includes('--prerelease')) throw new Error('candidate publication must not use GitHub prerelease flag');
if(releaseWorkflow.includes('--prerelease=false')) throw new Error('stable promotion must be evidence-only, not prerelease flag mutation');
